...
local function process()
  ...
  local database_tables = {}
	...
	depositList = getDeposit()
	orderList,totalList = getOrderList()
...
local function CreateSchema()
  ...
  local database_tables = {
  {table_name_deposit,{'account_number' .. sql_column_text}},
  {table_name_order,{
...
local function getOrderListion(orders)
	local orderList= {}
	local totalList={}
	local total_comm_fee=0
	local total_vat=0
	local DECIDE_deposit_obj = fo.Deposit( tonumber(depositId) )
	for _,no in pairs(orders)
		local orderItem={}
		local order = fo.Order( no )
		local op = order:getOrderOperations()
		--local entry_name = op:getEntryUser()
